...
iniciar_cronometro:
    rcall debounce2     ; Debounce mais rápido para resposta ágil

    lds reg_temp, modo_atual
    cpi reg_temp, MODO_CRONOMETRO
    brne start_sair

    ; Verifica se o botão START está pressionado
    lds reg_temp, PINC_ADDR
    sbrs reg_temp, BOTAO_START
    rjmp start_botao_pressionado
    
    rjmp start_sair             ; Se não está pressionado, sai

start_botao_pressionado:
    ; Espera um tempo mínimo (20ms) para confirmar o pressionamento
    ldi reg_temp, 20
start_aguarda_confirma:
    dec reg_temp
    brne start_aguarda_confirma

    ; Verifica se ainda está pressionado após o tempo mínimo
    lds reg_temp, PINC_ADDR
    sbrs reg_temp, BOTAO_START
    rjmp start_confirma_pressionado
    
    rjmp start_sair             ; Se não está mais pressionado, ignora

start_confirma_pressionado:
    ; Alterna estado imediatamente (não espera soltar)
    ...
    ; Espera soltar o botão antes de aceitar novo input
